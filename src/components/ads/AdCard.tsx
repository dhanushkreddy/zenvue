...
export function AdCard({ ad, onConvert, isConverted, layout = 'default' }: AdCardProps) {
  const { rateAd, getRating } = useAdStore();
    ...
    return (
        <div className="flex items-center space-x-4 p-2 rounded-lg hover:bg-accent transition-colors">
            ...
            {isConverted ? (
                ...
                <Button size="sm" variant="ghost" onClick={onConvert}>
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      ...
      <CardFooter className="p-2 bg-muted/30 border-t flex flex-col items-stretch gap-2">
        ...
        <Button onClick={onConvert} disabled={isConverted} className="w-full">
            {isConverted ? (
                <>
